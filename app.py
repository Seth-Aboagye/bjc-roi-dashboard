# app.py (ONLINE VERSION — Dashboard only)
# - CSV upload
# - Interactive filters + charts
# - NO Excel/Word/PowerPoint exports (to avoid Streamlit Cloud build issues)
...
st.title("BJC Fundraising ROI Dashboard (Online - Dashboard Only)")
st.caption(
    "Upload Donations + Costs CSV to compute ROI and explore trends. "
    "Report exports (Excel/Word/PPT) are disabled in the online version for deployment stability."
)
...
try:
    ...
    st.error(f"Could not read one of the CSV files. Details: {e}")
    st.stop()
...
campaigns = sorted(
    set(donations_df["campaign_code"].dropna().unique()).union(set(costs_df["campaign_code"].dropna().unique()))
)
...
# Filtered data
d = donations_df[
    (donations_df["date"] >= start_dt)
    & (donations_df["date"] <= end_dt)
...
c = costs_df[
    (costs_df["date"] >= start_dt)
    & (costs_df["date"] <= end_dt)
    & (costs_df["channel"].isin(sel_channels))
    & (costs_df["campaign_code"].isin(sel_campaigns))
].copy()

if d.empty and c.empty:
    st.warning("No data matches the filters. Try widening the date range or selecting more campaigns/channels.")
    st.stop()
...
# -----------------------------
# Online notice (no exports)
# -----------------------------
st.markdown("---")
st.info(
    "✅ Online version is **dashboard-only** (no Excel/Word/PPT downloads) to ensure Streamlit Cloud deployment stability. "
    "Use the local version for full report generation."
)
